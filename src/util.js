...
const readJson = (filename) => {
    return JSON.parse(fs.readFileSync(`${jsonPath}/${filename}.json`, 'utf8'))
}

const saveJson = (filename, jsonData) => {
    if (!fs.existsSync(jsonPath))
        fs.mkdirSync(jsonPath)
    fs.writeFile(`${jsonPath}/${filename}.json`, JSON.stringify(jsonData, null, 2), e => { if (e) console.log(e) })
}

const xlsxToJson = (filename) => {
    console.log('# Carregando arquivo XLSX')
    const workbook = xlsx.readFile(`${xlsxPath}/${filename}.xlsx`)
...
const saveXlsx = (filename, dataframe) => {
    ...
    xlsx.writeFile(workbook, `${xlsxPath}/${filename}.xlsx`)
}
